...
| `Dialog`                      | `open`, `title`, `handleClose`, `initialFocus`, `closeOnBackdropClick`, `mobileFullScreen`, `containerClassName`, `className`, `animationClass`                                  | Base modal. Backdrop click is disabled by default; set `closeOnBackdropClick` to `true` to close on outside click. On mobile, browser/device Back closes the topmost dialog before route navigation. `initialFocus="first-input"` focuses the first enabled input/textarea/select; `initialFocus="submit"` focuses the primary submit action. |
